  ...
  return <>
    <header className="sticky top-0 z-50 border-b border-white/60 bg-white/85 backdrop-blur-xl">
      <div className="container flex h-[76px] items-center justify-between gap-8">
        ...
        <div className="hidden lg:block"><a href={whatsappLink("Hello! I'd like to book a free demo class.")} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-2 rounded-full bg-[#101b36] px-5 py-3 text-[13px] font-bold text-white transition hover:bg-blue-600">Book a Free Demo <ArrowUpRight size={16} className="transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5" /></a></div>
      ...
      {open && <nav className="border-t border-slate-100 bg-white px-5 pb-5 pt-3 lg:hidden">{navItems.map(item => <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className="block border-b border-slate-100 py-3 text-sm font-semibold text-slate-700">{item.label}</Link>)}<a href={whatsappLink("Hello! I'd like to book a free demo class.")} target="_blank" rel="noopener noreferrer" onClick={() => setOpen(false)} className="mt-4 flex justify-center rounded-full bg-blue-600 px-5 py-3 text-sm font-bold text-white">Book a Free Demo</a></nav>}
    </header>
    <main>{children}</main>
    ...
    <a href={whatsappLink("Hi, I'd like to chat with you.")} target="_blank" rel="noopener noreferrer" className="fixed bottom-6 right-6 z-50 flex items-center gap-2.5 rounded-full bg-[#25D366] px-4 py-3 text-sm font-bold text-white shadow-lg shadow-[#25D366]/25 transition hover:scale-105">
